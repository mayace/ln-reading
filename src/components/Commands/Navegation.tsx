...
export function NavigationCommand({ navigation, onTryToChange }: INavigationProps) {
  ...
  return (
    <div className="item">
      <button
        onClick={() => onChange({ pageI: getValidNumber(navigation.pageI - 1) })}
        type="button"
      >
        &lt;
      </button>
      <input
        onChange={({ target }) => onChange({ pageI: getValidNumber(parseInt(target.value) - 1) })}
        value={navigation.pageI + 1}
        className="counter"
        type="text"
      />
      <button onClick={() => onChange({ pageI: navigation.pageI + 1 })} type="button">
        &gt;
      </button>
      <div className="row-2">
        <label>
          <input
            onChange={({ target }) => onChange({ separator: target.value.trim() })}
        ...
        <input
          onChange={({ target }) => onChange({ length: getValidNumber(parseInt(target.value)) })}
